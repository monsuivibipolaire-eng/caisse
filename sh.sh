...
echo "🚑 Correction de l'erreur DatePipe (RuntimeError: NG02100)..."

# 1. Mise à jour du TypeScript (Ajout de la méthode parseDate)
# On garde la structure actuelle mais on ajoute la méthode de sécurité
...
# 2. Mise à jour du HTML (Utilisation de parseDate)
# On remplace le calcul manuel risqué par l'appel à parseDate
  ...
  <div class="flex flex-col items-center justify-center min-h-full p-4">
    ...
    <div id="receipt-area" class="bg-white text-slate-900 w-full max-w-[380px] p-6 shadow-2xl relative ticket-paper">
      ...
      <div class="text-center mb-6">
        ...
        <p class="text-xs font-mono text-slate-500 mt-1">{{ parseDate(sale.date) | date:'dd/MM/yyyy HH:mm' }}</p>
      </div>
...
echo "✅ Crash réparé : Le ticket accepte maintenant tous les formats de date."